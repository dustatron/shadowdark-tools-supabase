 ...
 */
export const DAMAGE_DICE = ['1', '1d2', '1d3', '1d4', '1d6', '1d8', '1d10', '1d12', '2d4', '2d6', '2d8', '2d10', '2d12', '3d6', '3d8', '4d6'] as const;
...
export function calculateXP(challengeLevel: number): number {
  if (challengeLevel < CHALLENGE_LEVELS.MIN || challengeLevel > CHALLENGE_LEVELS.MAX) {
    throw new Error(`Challenge level must be between ${CHALLENGE_LEVELS.MIN} and ${CHALLENGE_LEVELS.MAX}`);
  }
...
export function getDifficultyTier(challengeLevel: number): string {
  if (challengeLevel <= CHALLENGE_LEVELS.TIER_1.max) return CHALLENGE_LEVELS.TIER_1.name;
  if (challengeLevel <= CHALLENGE_LEVELS.TIER_2.max) return CHALLENGE_LEVELS.TIER_2.name;
  if (challengeLevel <= CHALLENGE_LEVELS.TIER_3.max) return CHALLENGE_LEVELS.TIER_3.name;
 ...
 */
export function suggestHitPoints(challengeLevel: number): { min: number; max: number; suggested: number } {
...
export function validateAttackBalance(
  ...
  return { isBalanced: true, message: 'Attacks are balanced for this challenge level' };
}

/**
 * Enhanced Zod schema for Shadowdark monster validation
 */
export const shadowdarkMonsterSchema = z.object({
  ...
  attacks: z.array(
    z.object({
      ...
      damage: z.string().refine(validateDamageString, {
        message: 'Invalid damage format. Use formats like "1d6", "2d8+2", or "10"',
      }),
  ...
  (data) => {
    ...
    return data.hit_points >= hpRange.min * 0.5 && data.hit_points <= hpRange.max * 2;
  },
  (data) => ({
