...
const SpellSchema = z.object({
  ...
  slug: z.string().min(1).max(100).regex(/^[a-z0-9_-]+$/, 'Slug must be lowercase with hyphens or underscores'),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const classes = searchParams.get('classes')?.split(',').filter(Boolean) || null;
    const durations = searchParams.get('durations')?.split(',').filter(Boolean) || null;
    const ranges = searchParams.get('ranges')?.split(',').filter(Boolean) || null;
    const sources = searchParams.get('sources')?.split(',').filter(Boolean) || null;

    // Use the search function if we have search parameters
    if (search || minTier || maxTier || classes || durations || ranges || sources) {
      ...
      const total = spells && spells.length > 0 ? Number(spells[0].total_count) : 0;
    ...
    const { data: spells, error, count } = await supabase
      .from('all_spells')
      .select('*', { count: 'exact' })
