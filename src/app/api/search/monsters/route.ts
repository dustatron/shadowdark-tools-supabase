...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!validatedParams.q && !validatedParams.minLevel && !validatedParams.maxLevel &&
        !validatedParams.types && !validatedParams.locations && !validatedParams.sources) {
    ...
    const { data: allResults, error: countError } = await supabase.rpc('search_monsters', {
      search_query: validatedParams.q || '',
      min_level: validatedParams.minLevel || 1,
      max_level: validatedParams.maxLevel || 20,
      monster_types: validatedParams.types || null,
