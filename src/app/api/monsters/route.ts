...
export async function GET(request: NextRequest) {
  try {
    ...
    const locations =
      searchParams.get("locations")?.split(",").filter(Boolean) || null;
    const sources =
    ...
    if (search || minLevel || maxLevel || types || locations || sources) {
      ...
      // Get total count for pagination (use the data length as count since we're getting all results)
      const count = monsters?.length || 0;
