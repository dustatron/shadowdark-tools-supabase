...
describe('Monster Management Integration Tests', () => {
  ...
  describe('Complete Monster Lifecycle', () => {
    it('should create, read, update, and delete a custom monster', async () => {
      ...
      expect(readResponse.data.challenge_level).toBe(newMonster.challenge_level);
      ...
      expect(updateResponse.data.challenge_level).toBe(updateData.challenge_level);
      ...
      const found = searchResponse.data.some((monster: any) => monster.id === createdMonsterId);
  ...
  describe('Monster List Integration', () => {
    it('should create list, add monsters, and export', async () => {
      ...
      expect(listWithMonstersResponse.data.user_list_monsters.length).toBeGreaterThan(0);
  ...
  describe('Search and Filter Integration', () => {
    ...
    it('should generate random monsters with filters', async () => {
      const randomResponse = await supabase
  ...
  describe('Database Performance Integration', () => {
    ...
    it('should handle pagination correctly', async () => {
      // Get first page
      const page1Response = await supabase
        .rpc('search_monsters', {
          search_query: '',
          min_level: 1,
          max_level: 10,
      ...
      // Get second page
      const page2Response = await supabase
        .rpc('search_monsters', {
          search_query: '',
          min_level: 1,
          max_level: 10,
