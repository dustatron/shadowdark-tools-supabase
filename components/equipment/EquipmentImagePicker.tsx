...
import {
  ...
  type DefaultEquipmentImage,
} from "@/lib/config/default-equipment-images";
...
export function EquipmentImagePicker({
  ...
  const handleFileChange = async (
    ...
    try {
      ...
      formData.append("folder", `shadowdark/equipment/${userId}`);
  ...
  const handleDefaultIconSelect = (image: DefaultEquipmentImage) => {
    if (disabled) return;
            ...
            {categories.map((category) => (
              <div key={category}>
                ...
                <div className="flex flex-wrap gap-2">
                  {DEFAULT_EQUIPMENT_IMAGES.filter(
