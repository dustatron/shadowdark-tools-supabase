...
describe('GET /api/monsters/random - Contract Tests', () => {
  ...
  it('should support filters parameter', async () => {
    ...
    const response = await fetch(`${API_BASE}/api/monsters/random?filters=${encodeURIComponent(filters)}`);
  ...
  it('should return 400 for invalid filter JSON', async () => {
    const response = await fetch(`${API_BASE}/api/monsters/random?filters=invalid-json`);
