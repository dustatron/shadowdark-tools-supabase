...
describe("Monster Search and Discovery - Integration Tests", () => {
  ...
  it("should perform end-to-end monster search workflow", async () => {
    ...
    const searchResponse = await fetch(
      `${API_BASE}/api/monsters?q=goblin&min_cl=1&max_cl=3`,
    );
    ...
    const detailResponse = await fetch(
      `${API_BASE}/api/monsters/${firstMonsterId}`,
    );
    ...
    const fuzzyResponse = await fetch(
      `${API_BASE}/api/monsters?q=gobln&fuzziness=high`,
    );
    expect(fuzzyResponse.status).toBe(200);

    // 4. Test random monster with filters
    const randomResponse = await fetch(
      `${API_BASE}/api/monsters/random?filters=${encodeURIComponent(
        JSON.stringify({
  ...
  it("should handle search with tag filtering", async () => {
    const response = await fetch(
      `${API_BASE}/api/monsters?tags=humanoid&type=official`,
    );
  ...
  it("should maintain search performance under load", async () => {
    const searches = Array(10)
      ...
      .map(async (_, i) => {
        ...
        const response = await fetch(
          `${API_BASE}/api/monsters?q=test${i}&limit=20`,
        );
